...
namespace GLSLPT {
    ...
    void Perspective(float fovyInDegrees, float aspectRatio, float znear, float zfar, float* m16) 
    {
        ...
        half_height = znear * tanf(Math::Radians(fovyInDegrees));
        half_width = aspectRatio * half_height;
        Frustum(-half_width, half_width, -half_height, half_height, znear, zfar, m16);
    }
    ...
    Camera& Camera::operator=(const Camera& other) {
        ptrdiff_t len = (unsigned char*)&isMoving - (unsigned char*)&position.x;
        isMoving = memcmp(&position.x, &other.position.x, len) != 0;
        memcpy(&position.x, &other.position.x, len);
        return *this;
    }
